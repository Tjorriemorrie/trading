...
def main():
    ...
    for currency in currencies:
        ...
        # show accuracy
        score = rfc.score(X=X_train, y=y_train)
        logging.info('RFC train Accuracy: {0}'.format(score))
        score = rfc.score(X=X_test, y=y_test)
        logging.info('RFC test Accuracy: {0}'.format(score))
        scores = cross_validation.cross_val_score(estimator=rfc, X=X_test, y=y_test)
        logging.info('CV Accuracy: {0} (+/- {1})'.format(scores.mean(), scores.std() * 2))

        # simulate
        trades = simulate(rfc, X_test, y_test)
        results[currency] = trades

    for currency, trades in results.iteritems():
        wins = sum([1. for t in trades if t])
        ratio = wins / len(trades)
        logging.info('{0} {1}'.format(currency, ratio))
...
def simulate(rfc, X_test, y_test):
    logging.info('Simulation: starting...')
    pb = ProgressBar(maxval=len(X_test)).start()
    trades = []
    for i, x in enumerate(X_test):
        ...
        # logging.info('{0} from predicted {1} vs actual {2}'.format(goal, predict, reward))
    ...
    wins = sum([1. for t in trades if t])
    ratio = wins / len(trades)
    logging.info('Ratio: {0}'.format(ratio))
