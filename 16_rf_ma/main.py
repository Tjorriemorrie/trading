...
def summarizeActions(q):
    ...
    for (state, action), value in q.iteritems():
        action_info = summary.get(action, {'values': []})
        action_info['values'].append(value)
        summary[action] = action_info

    for action, info in summary.iteritems():
        logging.error('{0:10s} after {2} states with {1:.4f} avg'.format(action, np.mean(info['values']), len(info['values'])))
