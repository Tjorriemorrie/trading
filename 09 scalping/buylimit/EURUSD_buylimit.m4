/*
my eurusd 1min buy limit trader
*/
...
extern double STOP_LOSS = 450.0;
extern double ENTRY = 20.0;
extern double WAIT_FOR = 340.0;
...
int CreateTrade() {
    ...
    while (true) {
        ...
        // get enter at
        double EnterAt = Bid - ENTRY * Point;
        double EnterMax = Ask - market_stop_level * Point;
        if (Price > PriceMinimum) {
            Alert("Entry price too low (", EnterAt, ") changed to ", EnterMax);
            EnterAT = EnterMax;
        }
        EnterAt = NormalizeDouble(EnterAt, Digits);

         // get stop loss
         double SL = Bid - STOP_LOSS * Point;
         if (STOP_LOSS < market_stop_level) {
            SL = Bid - market_stop_level * Point;
         ...
         if (TAKE_PROFIT < market_stop_level) {
             TP = Bid + market_stop_level * Point;
         ...
         int ticket = OrderSend(symbol, OP_BUYLIMIT, lot, EnterAt, 0, SL, TP, NULL, 0, expire_at);

         // success
         if (ticket > 0) {
            Alert("Placed order Buy Limit ", ticket);
            break;
         }
